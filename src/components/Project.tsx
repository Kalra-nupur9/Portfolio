      ...
      <div className="project-grid">
        {visibleProjects.map((project) => (
          <div key={project.id} className="project-card">
            ...
            <img src={project.image} alt={project.name} loading="lazy" className="project-img" />
            <div className="z-10 flex flex-grow flex-col p-5">
              <div className="py-2">
                <h3 className="h3-project">{project.name}</h3>
              </div>

              <p className="para project-details">{project.description}</p>
              <div className="flex-grow" />
              <div className="mt-3 flex flex-wrap gap-2">
                {project.techStack.map((tech, index) => (
                  <span
                    key={index}
                    className="rounded-full border border-cyan-400/30 bg-cyan-500/10 px-3 py-1 text-xs text-cyan-300 capitalize"
                  >
                    {tech}
                  </span>
                ))}
